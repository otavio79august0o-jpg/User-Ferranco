...
- GET  /public-key
- GET  /healthz
- POST /admin/users/create     (admin) -> cria usuário e retorna activation_code (informativo)
- GET  /admin/users/list       (admin)
- POST /admin/users/disable    (admin)
- POST /admin/users/reset-pass (admin)
- POST /login                  (client) -> username+password -> auth_code (one-time, curto prazo)
- POST /claim-auth             (client) -> auth_code+installation_id -> refresh_token (vincula PC)
...
"""
import os, json, base64, hashlib, secrets, string
...
def admin_create_user(req: UserCreate, x_api_key: str = Header(default='')):
    ...
    db['users'][req.username] = {
        "pwd": pwd, "cnpj": req.cnpj, "company_name": req.company_name,
        "range_mode": req.range_mode, "fixed_min": int(req.fixed_min), "fixed_max": int(req.fixed_max), "delta": int(req.delta),
        "require_sum_code": bool(req.require_sum_code), "letter_values": LETTER_VALUES_DEFAULT,
        "features": req.features or {}, "status": req.status, "created_at": datetime.utcnow().isoformat()+'Z',
...
def admin_list_users(x_api_key: str = Header(default='')):
    if x_api_key != ADMIN_API_KEY: raise HTTPException(401, 'unauthorized')
    db = load_db()
    out = {u: {k:v for k,v in rec.items() if k not in ('pwd',)} for u,rec in db['users'].items()}
...
def admin_disable_user(req: UserDisable, x_api_key: str = Header(default='')):
    ...
    if not rec: raise HTTPException(404, "user not found")
...
def login(req: LoginReq):
    ...
    db['auth_codes'][code] = {
        "username": req.username, "created_at": datetime.utcnow().isoformat()+'Z',
        "expires_at": (datetime.utcnow()+timedelta(minutes=10)).isoformat()+'Z', "status": "pending"
    }
    save_db(db)
...
def claim_auth(req: ClaimAuthReq):
    ...
    if ac.get('status') != 'pending': raise HTTPException(400, "auth_code already used")
    try:
    ...
    db['tokens'][token] = {
        "username": ac['username'], "cnpj": user['cnpj'], "company_name": user.get('company_name'),
        "installation_id": req.installation_id,
        "range_mode": user['range_mode'], "fixed_min": int(user['fixed_min']), "fixed_max": int(user['fixed_max']), "delta": int(user['delta']),
        "letter_values": user['letter_values'], "require_sum_code": bool(user['require_sum_code']),
        "features": user.get('features', {}), "status": "active", "created_at": datetime.utcnow().isoformat()+'Z'
    }
    ac['status'] = 'used'; ac['used_at'] = datetime.utcnow().isoformat()+'Z'; ac['used_by_installation_id'] = req.installation_id
    save_db(db)
...
class IssueReq(BaseModel):
    cnpj: str; company_name: Optional[str]=None; installation_id: str; ym: Optional[str]=None
    range_mode: Literal['fixed','delta']='fixed'
    fixed_min: Optional[int]=310; fixed_max: Optional[int]=315; delta: Optional[int]=3
    letter_values: Optional[dict]=None; require_sum_code: bool=True; features: Optional[dict]=None; expires_at: Optional[str]=None
    ...
    def _check_fixed(self):
        if self.range_mode == 'fixed':
            if self.fixed_min is None or self.fixed_max is None: raise ValueError('fixed_min/fixed_max obrigatórios')
            if self.fixed_min > self.fixed_max: raise ValueError('fixed_min <= fixed_max')
        return self
